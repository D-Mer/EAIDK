...
{        
    ...
    int mode = ONE_TO_N;
    struct face *face_db;
	struct face *face_db_head;
	struct face face_entity;
    int photo_max = 0;
    fcv::Point mode_p;
    fcv::Point name_p;
    char mode_show[30];
    ...
    if (mode == ONE_TO_ONE)
    {
		face_entity = get_face_from_db_by_id(mysql, primary_key);
		face_db_head = &face_entity;
		if (face_db_head->feature == NULL){
			printf("db FACE is empty!\n");
			release_tengine_library();
			mysql_close(mysql);
			return -1;
		}
		face_show = fcv::imread(face_db_head->photo_path);
    ...
    {
        int i = 0;
		face_entity = get_face_from_db_by_id(mysql, 1);
		face_db_head = &face_entity;
		if (face_db_head->feature == NULL){
			printf("db FACE is empty!\n");
			release_tengine_library();
        ...
        for (i = 1; i< PHOTO_NUM_MAX; i++){
			face_entity = get_face_from_db_by_id(mysql, i+1);
			face_db = &face_entity;
			if (face_db->feature == NULL)
            {
                break;
            }
			insert(face_db_head, face_db);
        }
    ...
    while(1) {
        ...
        if (mode == ONE_TO_ONE_POLLING)
        {
            ...
            if (((t_polling_end.tv_sec * 1000000 + t_polling_end.tv_usec) - (t_polling_start.tv_sec * 1000000 + t_polling_start.tv_usec)) / 1000 > 10*1000)
            {
				...
				face_db = get(face_db_head, index);
                face_show = fcv::imread(face_db->photo_path);
                if (face_show.empty())
                {
        ...
        if (mode == ONE_TO_ONE || mode == ONE_TO_ONE_POLLING){
			similarity_max = cosine_dist(feature, face_db_head->feature, FEATURESIZE);
        }
        ...
        else if (mode == ONE_TO_N)
        {
			...
			face_db = face_db_head;
            while(face_db != NULL)
            {
                similarity = cosine_dist(feature, face_db->feature, FEATURESIZE);
        ...
        {   
            ...
            if(mode == ONE_TO_N)
            {
				face_db = get(face_db_head, index_max);
                face_show = fcv::imread(face_db->photo_path);
                if (face_show.empty())
                {
                     printf("read photo[%s] fail!\n",face_db->photo_path);
